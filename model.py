...
class ACRNNBaselineStable(nn.Module):
...
class ACRNN_LockedDropout_WGTInit(nn.Module):
    ...
    def __init__(self, num_classes=4, is_training=True,
        ...
        self.rnn2 = nn.LSTM(input_size=self.cell_units * 2, hidden_size=self.cell_units, 
                            batch_first=True, num_layers=1, bidirectional=True)

        self.rnn3 = nn.LSTM(input_size=self.cell_units * 2, hidden_size=self.cell_units, 
                            batch_first=True, num_layers=1, bidirectional=True)

        self.rnn4 = nn.LSTM(input_size=self.cell_units * 2, hidden_size=self.cell_units, 
                            batch_first=True, num_layers=1, bidirectional=True)
    ...
    def _initialize_weights(self):
        ...
        for name, param in self.rnn4.named_parameters():
            if name.startswith('weight'):
                nn.init.orthogonal_(param)
            else:
                nn.init.zeros_(param)
